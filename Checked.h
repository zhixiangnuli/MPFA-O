...
constexpr int nxx = 3, nyy = 3, nzz = 3; // 细分前
// constexpr int nxx = 10, nyy = 9, nzz = 8;                          //细分前
...
void _multcross(const double *x, const double *y, double *z) // 叉乘
{
    z[0] = x[1] * y[2] - x[2] * y[1];
    z[1] = x[2] * y[0] - x[0] * y[2];
    z[2] = x[0] * y[1] - x[1] * y[0];
...
{
    double id = 1.0 * i, jd = 1.0 * j, kd = 1.0 * k, divnd = 1.0 * divn; // 转化成double
