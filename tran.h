...
/*
 *
 *  对于不使用调和平均的部分，忽略分量，此处会产生一定量的结果偏移
 *
 */
              ...
              const double *verts, int idx_row, double para_cell[8], int idx_cell[8], int divn)
{
    ...
    double c[8][3];      // 8个网格中心点坐标
    double p[12][3];     // 12个连续点坐标
    double n[12][3];     // 12个连续点对应网格交接面的法向量
    double pp[6][3];     // 12个交接面与坐标轴的6个交点
    double S[12];        // 12个交接面面积
    double w[12][3];     // 12个交接面构成方程的左边各项系数
    double w_rvs[12][3]; // 12个交接面构成方程的右边各项系数
    int idx_c[8][3];     // 8个网格对应的网格控制区域的三个p[i][3]连续点编号i
    int idx_w[12];       // 12个交接面构成方程的左边的c[i][3]网格编号i
    int idx_w_rvs[12];   // 12个交接面构成方程的右边的c[i][3]网格编号i
